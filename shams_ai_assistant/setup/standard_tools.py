...
def sync_standard_tools():
    ...
    for i, (name, label, desc, cat, op, handler, params) in enumerate(TOOLS, 1):
        exists = frappe.db.exists("AI Tool", name)
        if exists:
            doc = frappe.get_doc("AI Tool", exists)
            # Synchronize standard metadata only. Preserve enabled state, confirmations,
            # roles, allowed DocTypes, limits and other administrator choices.
            doc.update({
                "tool_label": label,
                "description": desc,
                "is_standard": 1,
                ...
                "read_only": 0 if name in WRITE else 1,
                "priority": i * 10,
            })
            doc.set("parameters", [])
        else:
            doc = frappe.get_doc({
                ...
                "tool_label": label,
                "description": desc,
                "enabled": 0 if name == "delete_document" else 1,
                "is_standard": 1,
                "category": cat,
                "operation": op,
                "handler_method": handler,
                "read_only": 0 if name in WRITE else 1,
                "requires_confirmation": 1 if name in WRITE else 0,
...
def execute():
    """Compatibility entry point for manual execution."""
    sync_standard_tools()
