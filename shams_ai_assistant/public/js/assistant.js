(() => {
  ...
  function panelMarkup() {
    return `
      ...
      <aside id="shams-ai-panel" dir="rtl" aria-label="Shams AI Assistant">
        <div class="shams-ai-topbar">
          <div class="shams-ai-topbar-title">
            <span class="shams-ai-topbar-spark">✦</span>
            <b>Shams AI</b>
            <small id="shams-ai-model">مساعد ERPNext</small>
          </div>
          <div class="shams-ai-topbar-actions">
            <button id="shams-ai-new" type="button" aria-label="محادثة جديدة" title="محادثة جديدة">+</button>
            <button id="shams-ai-settings-button" type="button" aria-label="الإعدادات" title="الإعدادات">⚙</button>
            <button id="shams-ai-close" type="button" aria-label="إغلاق" title="إغلاق">×</button>
          </div>
        </div>
        <button id="shams-ai-current-config" type="button" class="shams-ai-config-summary" aria-label="فتح الإعدادات" title="الإعدادات الحالية">
          <span id="shams-ai-current-provider">لا يوجد مزود</span>
          <small id="shams-ai-current-mcp">بدون MCP</small>
        </button>
        <section id="shams-ai-settings" class="shams-ai-settings" aria-hidden="true">
          <div class="shams-ai-settings-head">
            <b>إعدادات المحادثة</b>
            <button id="shams-ai-settings-close" type="button" aria-label="إغلاق الإعدادات">×</button>
          </div>
          <label>المزود والموديل
            <select id="shams-ai-provider" aria-label="المزود والموديل"></select>
          </label>
          <label>خادم MCP
  ...
  function mount() {
    ...
    byId("shams-ai-button").addEventListener("click", () => setOpen(!state.open));
    ...
    byId("shams-ai-settings-button").addEventListener("click", () => setSettingsOpen(true));
    byId("shams-ai-current-config").addEventListener("click", () => setSettingsOpen(true));
    byId("shams-ai-settings-close").addEventListener("click", () => setSettingsOpen(false));
  ...
  function setOpen(open) {
    ...
    if (state.open) {
      setTimeout(() => byId("shams-ai-input")?.focus(), 120);
    } else {
  ...
  function setSettingsOpen(open) {
    const settings = byId("shams-ai-settings");
    if (!settings) return;
    const isOpen = Boolean(open);
    settings.classList.toggle("open", isOpen);
    settings.setAttribute("aria-hidden", isOpen ? "false" : "true");
  }
  ...
  function updateConfigSummary() {
    const provider = state.providers.find((item) => item.name === state.provider);
    const server = state.mcpServers.find((item) => item.name === state.mcpServer);
    const providerLabel = byId("shams-ai-current-provider");
    const mcpLabel = byId("shams-ai-current-mcp");
    if (providerLabel) providerLabel.textContent = provider
      ? `${provider.provider_name} — ${provider.default_model}`
      : "لا يوجد مزود";
    if (mcpLabel) mcpLabel.textContent = server
  ...
  async function loadMCPServers() {
    ...
    try {
      ...
      select.addEventListener("change", (event) => {
        state.mcpServer = event.target.value;
        updateConfigSummary();
      });
    } catch (error) {
